...
additional command line arguments
  ...
  --unpickle            if it exists, a pickle files generated in a previous run for this \<filename\> and this 
  ...
  --clobber             If set to true, will overwrite existing output files without asking. Default False.
  ...
  --asciiout            write distribution medians and 66% inclusion regions in an ascii output (default is not
                        to)
                        
  --asciidistrib         write the entire distribution for every scale in an ascii file output (default is not to)
                        
                        
  --multiproc (--multi)           multiprocess, with number of threads nps=max(available cores-1, MAXPROCESSES)

  --log LOGFILE         outputting messages to a log file instead of standard output. disabled when multiprocessing
