...
\begin{abstract}
We present the open-source Python code \verb=pyMCZ= that determines oxygen abundance and its error distribution via strong emission lines in the standard scales, based on the original IDL-code in \citet{kewley02} with updates from \citet{kewley08}, and expanded to include more recently developed scales. 
%MM: I changed the wording since it had passive voice and "determination of estimator of É" was a bit too windy.
The standard strong-line diagnostics have been used in many areas of astrophysics, including galaxy evolution and supernova (SN) host galaxy studies, to estimate the oxygen abundance in the interstellar medium through various emission line ratios. 
...
Here we introduce the open-source Python package named \textbf{pyMCZ}.
