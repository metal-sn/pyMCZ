...
%MM: no I don't think it has anything to to do with it since David is talking about stellar atmospheres of stars, not HII regions.
%
...
%%MM: are you sure that R23 is constrained to be LARGER than 0.9 and not SMALLER (and I think it's the log of R23)
...
%MM: Actually no, this is incorrect - PP04 does NOT use R23. It's just a linear relationship b/w metallicity from Te and the line ratio and it's NOT double valued.  I corrected it.
%We use [OIII]/\hb, and  [NII]/\ha~ to disctiminate between upper and lower branch \citep{kewley08}. \textbf{CHECK WITH FINAL CODE  by looking at the code}
...
We remind the user that the appropriate number of $N$ samples will depend on the diagnostic, and on the input data. The way in which the lines are combined, which is in many cases non-linear, will affect the shape of the distribution, and thus the number of datapoints 
that are needed to fully characterize it. 
...
We ran a bench mark calculation on a MacBook Air with an Intel Core i7 (1.7~GHz) and 8GB of 1600 MHz DDR3 memory. The dataset we used for the calculation includes 2 sets of line measurements (measurement 1, or M1, and measurement 2, M2). The flux values and their associated errors are shown in Table~\ref{tab:exampledata}. The code performs simple algebraic operations on large data arrays. It is vectorized along the sample-size dimension, so that the code loops over the smaller dimension corresponding to the measurements in input (2 in our example data), except when iterations are required by a metallicity scale algorithm (as in the KD02 scales).
...
With full graphic output (all histograms are being plotted) and performing all possible calculations (except those of D13  \emph{pyqz} ) for these data sets, the time required by the code is $\sim21.5$ seconds (wall clock time), less than 21 seconds of actual CPU time, and less then a second of CPU time spent in the kernel within the process. Adding the scales of D13 \emph{pyqz} for the same datasets, the run time becomes $\sim44$ seconds, with  $\sim43$ seconds of actual CPU time. For a single set of line measurements, the time drops to less than 11 seconds of CPU time. 
%% MM: not sure I understand the math here: running for all scales, including pyqz, on 2 example data sets, you have 43 seconds of CPU time, OK. Now for the 11 seconds of CPU time, I assume you ran again all scales (including D13) , but this time for only 1 measurement and you get a reduction factor of almost 4? That's a lot! Or is the 11 seconds of CPU time for all scales except D13?
...
%    - I think we should mention that this is for N=2000 samples. I added that above
...
%    - what do you mean by " extending for the duration of the function run. "? Isn't that all for the duration of the function run
...
CPU PLOT?
%%MM: I don't think a CPU plot is needed
